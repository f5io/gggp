...
const getProtos = (protoPath, ...protos) => {
  ...
  const saveDependencies = ({ links, dependencies, path }) =>
    localConfig.get()
  ...
  return all(...retrievers)
    ...
    .then(deps =>
        protoPath
          ? { deps, protoPath }
          : localConfig.get()
              .then(({ path: protoPath = paths.local.protos }) => ({ protoPath, deps })))
    .then(({ deps, protoPath }) => {
      ...
      return saveDependencies({ links, dependencies, path: protoPath })
        .then(() => ({ res: deps, protoPath }));
    })
    .then(({ res, protoPath }) => {
